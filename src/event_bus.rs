...
impl EventDispatcher {
    pub fn new(event_bus: Arc<TokioEventBus>) -> Self {
        Self { event_bus, ready_notify: None, is_ready: Arc::new(AtomicBool::new(false)) }
    }

    pub fn with_ready_notify(event_bus:Arc<TokioEventBus>,notify:Arc<Notify>)->Self{
        Self { event_bus, ready_notify: Some(notify), is_ready: Arc::new(AtomicBool::new(false)) }
    }
...
mod tests {
    ...
    async fn test_event_bus_unsubscribe() {
        ...
        let _ = event_bus.publish(DomainEvent::PriceUpdate(PriceUpdateEvent {
            symbol: "BTCUSDT".to_string(),
            price: 50000.0,
            price_change_pct_24h: 2.5,
            timestamp: 1234567890000,
