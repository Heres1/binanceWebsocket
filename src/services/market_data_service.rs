...
use crate::events::{
    AggTradeEvent, BookTickerEvent, DomainEvent, KlineCompletedEvent, PriceUpdateEvent,
};
...
use tokio_tungstenite::{
    tungstenite::handshake::client::generate_key, tungstenite::http, tungstenite::Message,
    WebSocketStream,
};
use url::Url;
...
impl MarketDataService {
    ...
    async fn connect_and_run(&self) -> Result<(), DomainError> {
        ...
        if env::var("USE_SSH_TUNNEL").is_ok() {
            log::info!("使用SSH隧道模式");
            let tls_stream = self
                .connect_direct_with_tls_host("127.0.0.1", 9443, "stream.binance.com")
                .await?;
            ...
            .await
            .map_err(|_| {
                DomainError::Service(ServiceError::MarketData("WebSocket握手超时".to_string()))
            })?
            .map_err(|e| {
        ...
        let use_proxy = match self.connection_mode {
            ...
            ConnectionMode::Auto => {
                env::var("HTTPS_PROXY").is_ok() || env::var("https_proxy").is_ok()
            }
        };
        ...
        if use_proxy {
            let proxy = env::var("HTTPS_PROXY")
                ...
                .map_err(|_| {
                    DomainError::Service(ServiceError::MarketData(
                        "代理模式但未设置 HTTPS_PROXY".to_string(),
                    ))
                })?;
            log::info!("使用HTTP代理: {}", proxy);
            let tls_stream = self
                .connect_via_proxy(&proxy, "stream.binance.com", 443)
                .await?;
            let ws_request = self.build_ws_request()?;
    ...
    fn build_ws_request(&self) -> Result<http::Request<()>, DomainError> {
        ...
        let request = http::Request::builder()
            ...
            .body(())
            .map_err(|e| {
                DomainError::Service(ServiceError::MarketData(format!("构建请求失败: {}", e)))
            })?;

        log::info!(
            "WebSocket请求头 | Host: {} | URI: {} | Origin: https://stream.binance.com",
            host,
            self.ws_url
        );
    ...
    async fn connect_direct_with_tls_host(
        &self,
        connect_host: &str,
        port: u16,
        tls_host: &str,
        ...
        .map_err(|e| {
            DomainError::Service(ServiceError::MarketData(format!("TCP连接失败: {}", e)))
        })?;
    ...
    async fn connect_via_proxy(
        ...
        .map_err(|e| {
            DomainError::Service(ServiceError::MarketData(format!("代理TCP连接失败: {}", e)))
        })?;
        ...
        .await
        .map_err(|_| {
            DomainError::Service(ServiceError::MarketData("读取代理响应超时".to_string()))
        })?
        .map_err(|e| {
            DomainError::Service(ServiceError::MarketData(format!("读取代理响应失败: {}", e)))
        })?;
    ...
    async fn handle_message(&self, text: &str) -> Result<(), DomainError> {
        ...
        match event_type {
            Some("kline") => {
                ...
                self.event_bus
                    .publish(DomainEvent::KlineCompleted(event))
                    .await
                    .map_err(|e| {
                        DomainError::Service(ServiceError::MarketData(format!(
                            "发布事件失败: {}",
            ...
            Some("aggTrade") => {
                ...
                self.event_bus
                    .publish(DomainEvent::AggTrade(event))
                    .await
                    .map_err(|e| {
                        DomainError::Service(ServiceError::MarketData(format!(
                            "发布事件失败: {}",
            ...
            Some("24hrTicker") => {
                ...
                self.event_bus
                    .publish(DomainEvent::PriceUpdate(event))
                    .await
                    .map_err(|e| {
                        DomainError::Service(ServiceError::MarketData(format!(
                            "发布事件失败: {}",
            ...
            _ => {
                    ...
                    || data.get("b").is_some() && data.get("a").is_some() && data.get("s").is_some()
                {
                    ...
                    self.event_bus
                        .publish(DomainEvent::BookTicker(event))
                        .await
                        .map_err(|e| {
                            DomainError::Service(ServiceError::MarketData(format!(
                                "发布事件失败: {}",
    ...
    fn parse_kline(&self, data: &serde_json::Value) -> Result<KlineCompletedEvent, DomainError> {
        let symbol = data.get("s").and_then(|v| v.as_str()).ok_or_else(|| {
            DomainError::Service(ServiceError::MarketData("缺少symbol字段".to_string()))
        })?;

        let k = data.get("k").ok_or_else(|| {
            DomainError::Service(ServiceError::MarketData("缺少k线数据字段".to_string()))
        })?;
        ...
        if close <= 0.0 || open <= 0.0 {
            log::warn!(
                "K线数据异常: symbol={} interval={} open={} close={}，跳过",
                symbol,
                interval,
                open,
                close
    ...
    fn parse_ticker(&self, data: &serde_json::Value) -> Result<PriceUpdateEvent, DomainError> {
        ...
        let price = data
            ...
            .and_then(|s| s.parse::<f64>().ok())
            .ok_or_else(|| {
                DomainError::Service(ServiceError::MarketData("缺少price字段".to_string()))
            })?;
