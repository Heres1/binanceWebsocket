...
**更新** 订单处理服务的日志级别已从INFO降级到DEBUG，以减少常规订单相关事件的日志冗余，同时保持调试信息的可用性。
...
subgraph "日志系统"
LOGGER["infrastructure/logging/logger.rs<br/>日志配置与级别"]
end
MAIN --> CMD_BUS
...
- [src/infrastructure/logging/logger.rs:45-131](file://src/infrastructure/logging/logger.rs#L45-L131)
...
- 日志系统
  - 支持TRACE、DEBUG、INFO、WARN、ERROR五个级别
  - 默认级别为INFO，可通过配置文件调整

**更新** 订单处理服务的日志级别已从INFO降级到DEBUG，以减少常规订单相关事件的日志冗余。
...
**更新** 订单生命周期管理中的日志级别已从INFO降级到DEBUG，以减少常规订单事件的日志输出。
...
- 日志级别
  - 订单提交、成交、取消事件使用DEBUG级别
  - 订单拒绝事件使用WARN级别

**更新** 订单状态跟踪中的日志级别已从INFO降级到DEBUG，以减少常规订单事件的日志输出。
...
- 性能优化
  ...
  - 合理设置事件缓冲容量与订阅过滤
- 日志优化
  - DEBUG级别日志减少常规订单事件的输出
  - 可通过配置文件调整全局日志级别

**更新** 订单查询接口、批量操作与性能优化中增加了日志优化相关内容。
...
LOGGER["infrastructure/logging/logger.rs"] --> MAIN
```
...
- 可观测性
  - 日志与错误聚合，便于定位性能瓶颈
  - DEBUG级别日志减少常规事件输出，提高性能
- 日志系统
  - 支持多种日志级别配置
  - 文件轮转和历史清理机制

**更新** 性能考虑中增加了日志系统和DEBUG级别日志的相关内容。
...
## 结论
本订单处理服务以事件驱动为核心，通过命令总线与事件总线实现解耦与高并发。当前实现了订单创建与事件发布，订单簿撮合、滑点与手续费等高级特性可按需扩展。日志级别已从INFO降级到DEBUG，有效减少了常规订单事件的日志冗余，同时保持了足够的调试信息。建议在后续迭代中补充订单簿数据结构、撮合引擎与资金管理模块，以完整覆盖订单生命周期。

**更新** 结论中反映了日志级别调整的改进效果。
...
- 如何调整日志级别？
  - 修改配置文件中的logging.level字段
  - 支持trace、debug、info、warn、error五个级别
- 为什么看不到订单事件日志？
  - 检查日志级别配置，确保设置为debug或更低级别
  - 确认日志文件路径和权限设置

**更新** 常见问题解答中增加了日志级别调整的相关问题。
