...
## 更新摘要
**变更内容**
- **关键参数优化**：止损比例、交易冷却时间、RSI阈值等参数已优化，提升了策略的风险管理能力
...
- [engine.rs:124-134](file://src/backtest/engine.rs#L124-134)
...
### 关键参数优化说明
**重要更新** 配置文件中的关键参数已优化，提升了策略的风险管理能力：

#### 止损比例优化
- **默认止损比例**：从 1.0% 优化为 0.8%
- **做空止损比例**：从 0.5% 优化为 0.5%（保持不变）
- **保本止损触发**：从 0.8% 优化为 0.3%（更积极的保护）
- **追踪止损触发**：从 1.0% 优化为 0.5%（更早激活）

#### 交易冷却时间优化
- **默认冷却时间**：从 600秒（10分钟）优化为 300秒（5分钟）
- **做空冷却时间**：从 14400秒（4小时）优化为 14400秒（保持不变）

#### RSI阈值优化
- **RSI超卖线**：从 50.0 优化为 30.0（更严格的超卖判断）
- **RSI超买线**：从 100.0 优化为 100.0（保持不变，禁用RSI出场）
- **做空RSI超卖线**：从 50.0 优化为 20.0（更积极的做空时机）

#### 追踪止损优化
- **追踪止损回撤距离**：从 0.8% 优化为 0.3%（更积极的保护）
- **做空追踪止损**：从 0.3% 优化为 0.3%（保持不变）
...
- [default.toml:30-53](file://config/default.toml#L30-L53)
...
## 结论
本文提供了在不同环境下部署与运行 Binance Rust 事件驱动交易系统的配置指南，重点覆盖网络连接模式、代理与 TLS、环境变量、日志与错误处理、回测系统和风控模块，并给出性能调优建议。回测系统支持四种模式，包括K线回测、参数优化、回放回测和V2多策略优化，现在还支持五种策略类型，提供趋势动量、EMA交叉、RSI反弹、突破策略和综合评分的全面验证能力。风控模块提供多层次风险控制，确保交易安全。

**重要更新**：配置文件中的关键参数已优化，包括止损比例（从1.0%降至0.8%）、交易冷却时间（从600秒降至300秒）、RSI阈值（从50.0降至30.0）等，显著提升了策略的风险管理能力和盈利能力。这些优化使得系统在保持较高胜率的同时，能够更好地控制下行风险，特别是在市场波动较大的情况下。

后续可在此基础上扩展订单执行模块，完善生产级监控与告警体系。
